<style>
    .li_paging{float:left; margin: 3px;list-style: none;border-radius: 25%;    text-align: center;}
    .a_paging{padding: 5px;    text-align: center; border-radius: 25%;}
    .span_paging{display:inline-block;    text-align: center; padding: 0px 3px; background: #009f75; color:white; border-radius: 25%; }
</style>
<?php
class Pagination
{
    ...
    {   
        ...
        if ($this->_config['total_record'] > $this->_config['limit'])
        {
            ...
            if ($this->_config['current_page'] > 1)
            {
                $p .= '<a class="a_paging" href="'.$this->__link('1').'"><<</a>';
                $p .= '<a class="a_paging" href="'.$this->__link($this->_config['current_page']-1).'"><</a>';
            }
            ...
            for ($i = $this->_config['min']; $i <= $this->_config['max']; $i++)
            {
                ...
                if ($this->_config['current_page'] == $i){
                    $p .= '<span class="span_paging">'.$i.'</span>';
                }
                else{
                    $p .= '<a class="a_paging" href="'.$this->__link($i).'">'.$i.'</a>';
                }
            }
            ...
            {
                $p .= '<a class="a_paging" href="'.$this->__link($this->_config['current_page'] + 1).'">></a>';
                $p .= '<a class="a_paging" href="'.$this->__link($this->_config['total_page']).'">>></a>';
            }
