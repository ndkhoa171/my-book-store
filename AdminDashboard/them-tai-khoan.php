...
if (isset($_POST['submit'])) {
    $query = "insert into accounts(AccountID  ,Password, Role) values('" . $_POST['accountIDText'] . "' ,'" . $_POST['passwordPassword'] . "', '" . $_POST['roleText'] . "')";
    if (!mysqli_query($conn, $query))
        echo $sql . "<br>" . mysqli_error($conn);
    else
        header("Location:" . "./them-tai-khoan.php");
}
                                ...
                                <div class="card-body align-items-center d-flex justify-content-center">
                                    <div class="pagination">
                                        ...
                                        if ($current_page > 1 && $total_page > 1) {
                                            echo '<a href="them-tai-khoan.php?page=' . ($current_page - 1) . '">&laquo</a>';
                                        }
                                        for ($i = $min; $i <= $max; $i++) {
                                            if ($i == $current_page) {
                                                echo '<a href="them-tai-khoan.php?page=' . $i . '" class="active">' . $i . '</a>';
                                            } else {
                                                echo '<a href="them-tai-khoan.php?page=' . $i . '">' . $i . '</a>';
                                            }
                                        }
                                        if ($current_page < $total_page && $total_page > 1) {
                                            echo '<a href="them-tai-khoan.php?page=' . ($current_page + 1) . '">&raquo</a>';
                                        }
                                        ?>
                                    </div>
                                </div>
                                ...
                                <div class="card-body">
                                    ...
                                    <form class="forms-sample" method="POST">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="accountIDText" required />
                                        </div>
                                        <div class="form-group">
                                            <label>Mật khẩu</label>
                                            <input type="password" class="form-control" name="passwordPassword" />
                                        </div>
                                        <div class="form-group">
                                            <label>Vai trò</label>
                                            <input type="text" class="form-control" name="roleText" required />
                                        </div>
...
<script>
    ...
    function cancel() {
        var accountIDText = document.getElementsByName("accountIDText");
        accountIDText.values = "";
        var passwordPassword = document.getElementsByName("passwordPassword");
        passwordPassword.values = "";
        var roleText = document.getElementsByName("roleText");
        roleText.values = "";
    }
</script>
