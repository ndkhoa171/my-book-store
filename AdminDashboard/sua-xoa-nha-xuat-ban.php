...
//PAGING
$result = mysqli_query($conn, 'select count(PublishingCompanyID ) as total from publishingcompanys');
...
$query_publishingcompanys = "select * from publishingcompanys limit ";
$query_publishingcompanys .= $start;
$query_publishingcompanys .= ",";
$query_publishingcompanys .= $limit;

//GET DATA
$publishingcompanys = mysqli_query($conn, $query_publishingcompanys);

//UPDATE
if (isset($_POST['edit'])) {
    $query_update = "update publishingcompanys set PublishingCompanyName= '" . $_POST['publishingCompanyNameText'] . "' where PublishingCompanyID= " . $_POST['publishingCompanyIDText'] . "";
    if (!mysqli_query($conn, $query_update))
        echo $sql . "<br>" . mysqli_error($conn);
    else
        header("Location:" . "./sua-xoa-nha-xuat-ban.php");
}

//DELETE
if (isset($_POST['delete'])) {
    $query_delete = "delete from publishingcompanys where PublishingCompanyID= " . $_POST['publishingCompanyIDText'] . "";
                ...
                <div class="content-wrapper">
                    <div class="row">
                                ...
                                <div class="card-body align-items-center d-flex justify-content-center">
                                    <div class="pagination">
                                        ...
                                        if ($current_page > 1 && $total_page > 1) {
                                            echo '<a href="sua-xoa-nha-xuat-ban.php?page=' . ($current_page - 1) . '">&laquo</a>';
                                        }
                                        for ($i = $min; $i <= $max; $i++) {
                                            if ($i == $current_page) {
                                                echo '<a href="sua-xoa-nha-xuat-ban.php?page=' . $i . '" class="active">' . $i . '</a>';
                                            } else {
                                                echo '<a href="sua-xoa-nha-xuat-ban.php?page=' . $i . '">' . $i . '</a>';
                                            }
                                        }
                                        if ($current_page < $total_page && $total_page > 1) {
                                            echo '<a href="sua-xoa-nha-xuat-ban.php?page=' . ($current_page + 1) . '">&raquo</a>';
                                        }
                                        ?>
                                    </div>
                                </div>
                        ...
                        <div class="col-lg-5 grid-margin stretch-card">
                            <div class="card">
                                <div class="card-body">
                                    <h4 class="card-title">CHỈNH SỬA NHÀ XUẤT BẢN</h4>
                                    <form class="forms-sample" method="POST">
                                        <div class="form-group">
                                            <label>Mã nhà xuất bản</label>
                                            <input type="text" class="form-control" name="publishingCompanyIDText" id="publishingCompanyIDText" required readonly />
                                        </div>
                                        <div class="form-group">
                                            <label>Tên nhà xuất bản</label>
                                            <input type="text" class="form-control" name="publishingCompanyNameText" id="publishingCompanyNameText" required />
                                        </div>
...
<script>
    $('.table tbody').on('click', '.btn-secondary', function() {
        ...
        $("#publishingCompanyIDText").val(col0.trim());
        $("#publishingCompanyNameText").val(col1.trim());
    })
